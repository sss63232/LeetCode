...
var findMinArrowShots = function (points) {
  ...
  for (let i = 1; i < points.length; i++) {
    const currentBalloon = points[i]
    const previousBalloon = points[i - 1]
    const isOverlappingWithPrvious = currentBalloon[0] <= previousBalloon[1]
    if (isOverlappingWithPrvious) {
      // treat the current and previous as a whole, update the xend
      currentBalloon[1] = Math.min(currentBalloon[1], previousBalloon[1])
    } else {
      arrows++
    }
  }
