...
const minDays = function (n) {
  const memo = new Map()
  const _getDays = oranges => {
    if (oranges === 0) return 0
    if (oranges === 1) return 1
    if (memo.has(oranges)) return memo.get(oranges)

    const ansDays = Math.min(
      oranges % 2 + _getDays(Math.floor(oranges / 2)) + 1,
      oranges % 3 + _getDays(Math.floor(oranges / 3)) + 1
    )

    memo.set(oranges, ansDays)
    return ansDays
  }
